...
class FlowAction extends CommonAction {
	...
	protected function _insert() {
		...
		if ($list !== false) {//保存成功
			...
			// dump(get_return_url());
	// 		die();
			$this -> success('新增成功OK!',get_return_url());
		} else {
			$this -> error('新增失败!');
			//失败提示
		}
	...
	public function del($id) {
		$isdo = M('flow')->where('id = '.$id)->max('step');
		if ($isdo >20) {
			$this -> assign('jumpUrl', get_return_url());
			$this -> success('审批已开始，无法删除！'.$isdo);
		}else {
			M('flow')->where('id = '.$id)->delete();
			M('flow_log')->where('flow_id = '.$id)->delete();
			$this -> assign('jumpUrl', get_return_url());
	...
	public function mark() {
		...
		switch ($action) {
			case 'approve' :
				...
				$list = $model -> save();
				$model = D("FlowLog");
				$model -> where("step=$step and flow_id=$flow_id and result = 3") -> delete();
	...
	public function approve() {
		...
		$model = D("FlowLog");
		$model -> where("step=$step and flow_id=$flow_id and result = 3") -> setField('is_del', 1);
