...
class WechatAction extends Action {
				...
				switch ($data ['Content']) {
					...
					default :
						...
						if (strpos ( $data ['Content'], '任务' ) !== false) {
							...
							$reply = array ( '欢迎使用教学评测OA服务号', 'text'  );
						}
						break;
				}
	...
	private function getSubscribe($openid = ''){
		$re = "";
		$re .= "您好，欢迎关注教学评测企业OA微信公众号。为了让您能方便快捷的使用教学评测企业OA";
		$re .= "请先确认您在教学评测OA系统中有帐号，并进行微信号码绑定，绑定后后可以直接进入教学评测OA";
		...
		if ($openid && $userid > 0) {
			switch ($taskevent) {
				...
				case 'ites_intro' : // 功能介绍
					$re = "";
					$re .= "您好，欢迎关注教学评测OA微信公众服务号。您可以回复以下数字了解您想了解的信息：\n";
					$re .= "1: 任务提醒\n";
					$re .= "2: 查看我申请的任务\n";
					$re .= "3: 查看邀请我的任务\n";
					$re .= "4: 查看待评价的任务\n";
