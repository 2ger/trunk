...
class shijianjxAction extends CommonAction {
	...
	public function index() {
		...
		// 通用流程
		$this->flow=M('flow')->where('type = 101 and (user_id='.$uid.' or confirm like "%'.$deptid.'%" or confirm like "%'.$uid.'%")')->select();
		//通用任务
		$this->task=M('task')->where('type = "shijianjx" and (user_id='.$uid.' or executor like "%'.$username.'%")')->select();
		//申报加分
