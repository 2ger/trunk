		...
		$.getJSON("<?php echo U('jsonall');?>", {
			...
			if (data != null) {
				$.each(data, function(i) {
					...
					html += ' <p id=' + data[i].id + ' class="" title="' + data[i].name + '">' +user_name(data[i].user_id) +' ' ; //event_msg
					html += data[i].name;
					html += '<a href="__APP__/Schedule/update/id/' + data[i].id + '/p/3" class="btn  btn-success btn-xs">标记为正常</a> <a href="__APP__/Schedule/update/id/' + data[i].id + '/p/5" class="btn btn-danger btn-xs">标记为异常</a> ';
					html += ' </p>';
					html += "</li>";
